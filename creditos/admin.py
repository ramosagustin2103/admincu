...
def enviar_json_liquidacion_expensas_pagas(modeladmin, request, queryset):
	for liquidacion in queryset:
		sehizoonosehizo = NewExp().request_inf_deuda(liquidacion)
		messages.success(request, sehizoonosehizo)

enviar_json_liquidacion_expensas_pagas.short_description = "Enviar json a Expensas Pagas"
...
class LiquidacionAdmin(admin.ModelAdmin):
	list_display = ['__str__', 'consorcio']
	list_filter = ['consorcio']
	actions = [procesar_liquidacion, procesar_liquidacion_exp, enviar_mail_liquidacion, eliminar_cupon_exp, obtener_barcode_exp_liquidacion, inf_deuda_2, enviar_json_liquidacion_expensas_pagas]
