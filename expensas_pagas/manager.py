...
class NewExp():
	...
	def formato_exp(self, tipo, long, val, format_date=False): #funcion que debe remplazarse al pasar a adminsmart
		...
		if tipo == 'char':
			char = str(valor)
			char_s_caracteres = re.sub('[/\*$()=~+%.#]', ' ', char)
			output = char_s_caracteres.ljust(long)[0:38]
		# if tipo is email output is empty
		if tipo == 'email':
			output = ""
			# if valor == None:
			# 	output = " "
			# else:
			# 	output = str(valor)[0:38]
		else:
			if tipo == 'float' and valor != 0:
		...
		for c in queryset:
			...
			if not vinculo == None:
				...
				if saldo1 != 0 or saldo2 !=0:
					if saldo1 < 100000 or saldo2 < 100000:
						if c.socio.socio.first():
							saldo1 = round(saldo1, 2) if saldo1 else None
							saldo2 = round(saldo2, 2) if saldo2 else None
							try: # Parche necesario porque en la base de dato existen socios sin usuario vinculado, no deberia corresponder en el nuevo sistema.
								email = c.socio.usuarios.first().email # Revisar, aveces manda 2 emails porque hay usuarios que tienen 2 emails cargados ejemplo (mendez mena)
							except:
								email = None
							...
							Propietario = self.formato_exp('char', 39, c.socio)
							Ubicacion = '1'
	...
	def inf_deuda_generator(self, queryset): #debera modificarse al pasar a adminsmart
		...
		"TotalesPrimerVencimiento":	round(self.total_1er_venc, 2),
		"TotalesSegundoVencimiento": round(self.total_2do_venc, 2),
		"Detalle": detalle
		}
		return t
	...
	def request_inf_deuda(self, liquidacion):
		cupones = DocumentoExp.objects.filter(documento__liquidacion=liquidacion)
		cupones.update(inf_deuda=False)
		queryset = Factura.objects.filter(liquidacion=liquidacion, receipt__total_amount__gt=1)
		self.codigo_cliente = liquidacion.consorcio.exp.first().codigo_exp
		payload = self.inf_deuda_generator(queryset)
		token = self.token
		url = 'https://api.expensaspagas.com.ar/InformeDeDeuda'
		head = {'Authorization': 'Bearer ' + token, 'Content-Type': 'application/json'}
